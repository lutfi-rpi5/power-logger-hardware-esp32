...
void StorageManager::_open(bool readOnly) const {
    _prefs.begin(PREF_NAMESPACE, readOnly);
}

void StorageManager::_close() const {
    _prefs.end();
}
...
void StorageManager::begin() {
    ...
    _open();
    // Touch namespace to ensure it exists
    _close();
    Serial.printf("[Storage] Ready. WiFi networks stored: %d\n", getWifiCount());
}
...
bool StorageManager::addWifiEntry(const char* ssid, const char* pass) {
    ...
    if (count >= MAX_WIFI_NETWORKS) {
        Serial.println("[Storage] WiFi list full");
        return false;
    }
    char sk[16], pk[16];
    ...
    _close();
    Serial.printf("[Storage] Added WiFi[%d]: %s\n", count, ssid);
    return true;
}
...
bool StorageManager::deleteWifiEntry(int index) {
    ...
    _prefs.putInt("wifi_count", count - 1);
    _close();
    Serial.printf("[Storage] Deleted WiFi[%d], count now %d\n", index, count - 1);
    return true;
}
...
void StorageManager::saveMQTTConfig(const MQTTConfig& cfg) {
    ...
    _close();
    Serial.printf("[Storage] MQTT config saved: %s:%d\n", cfg.server, cfg.port);
}
...
void StorageManager::saveThresholds(const ThresholdConfig& cfg) {
    ...
    _close();
    Serial.printf("[Storage] Thresholds saved: LOST=%.1f UNDER=%.1f OVER=%.1f UNBAL=%.2f%%\n",
...
void StorageManager::factoryReset() {
    Serial.println("[Storage] *** FACTORY RESET – clearing all NVS data ***");
    _open();
    _prefs.clear();
    _close();
    Serial.println("[Storage] Factory reset complete. Please reboot.");
}

// ─── Debug dump ───────────────────────────────────────────

void StorageManager::dump() const {
    Serial.println("=== StorageManager Dump ===");
    int n = getWifiCount();
    ...
    Serial.printf("MQTT: %s  port:%d  ssl:%d  user:%s\n",
        m.server, m.port, m.useSSL, m.user);
    CalibrationConfig c = getCalibration();
    for (int i = 0; i < 3; i++) {
        Serial.printf("Cal[%d]: V_offset=%.3f  I_offset=%.3f\n",
            i, c.voltageOffset[i], c.currentOffset[i]);
    }
    ThresholdConfig t = getThresholds();
    Serial.printf("Thresholds: LOST=%.1f  UNDER=%.1f  OVER=%.1f  UNBAL=%.2f%%\n",
